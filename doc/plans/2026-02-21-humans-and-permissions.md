...
- local mode that still feels instant (`npx cubecloud.io run` and go)
...
2. CLI command `pnpm cubecloud.io auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm cubecloud.io onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm cubecloud.io onboard`).
...
- expose key instance settings in API + CLI (`cubecloud.io configure` / `cubecloud.io onboard`)
...
7. `pnpm cubecloud.io onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
