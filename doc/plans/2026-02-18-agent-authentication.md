...
Agents need API keys to authenticate with CubeClawIO. The current approach
...
**Trust model:** The adapter process runs on the same machine as the CubeClawIO
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** CubeClawIO generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a CubeClawIO CLI
...
managed by the CubeClawIO server itself.
...
1. A company admin (user or agent) generates an **invite URL** from CubeClawIO.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The CubeClawIO SKILL.md (or a link to it)
   - What information CubeClawIO needs from the agent (e.g. webhook URL, adapter
...
5. CubeClawIO stores the pending registration.
...
7. On approval, CubeClawIO provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** CubeClawIO issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How CubeClawIO should send you heartbeats",
...
1. Generate a CubeClawIO invite link for the company.
...
4. A CubeClawIO company member approves the new agent.
5. CubeClawIO begins sending heartbeats to the OpenClaw webhook endpoint.
...
| **P3**   | CLI auth flow                     | `cubecloud.io auth login` for developer-managed remote agents.                                      |
