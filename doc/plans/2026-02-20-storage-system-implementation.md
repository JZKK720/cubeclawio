...
Add a single storage subsystem for CubeClawIO that supports:
...
- API serves attachment bytes through authenticated CubeClawIO endpoints.
...
- `cubecloud.io onboard` writes a valid `storage` config block by default.
- `cubecloud.io configure --section storage` can switch between local and s3 modes.
...
- `cubecloud.io doctor` reports actionable storage status.
