...
CubeClawIO needs to support two very different but equally valid ways of working:
...
Today, CubeClawIO already has the beginnings of this model:
...
- making CubeClawIO feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing CubeClawIO users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted CubeClawIO deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning CubeClawIO into a full code review product
...
This object must be recorded explicitly so that CubeClawIO can:
...
CubeClawIO should treat PRs as a type of work product linked back to:
...
CubeClawIO already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted CubeClawIO must be a first-class deployment mode

CubeClawIO cannot assume that it is running on the same machine as the code.

In cloud deployments, CubeClawIO may:
...
- `Runtime service`: a process or service CubeClawIO owns or tracks for a workspace
...
It also lets cloud-hosted CubeClawIO deployments point at codebases and remotes without pretending the CubeClawIO host has direct filesystem access.
...
Without an explicit `execution workspace` record, CubeClawIO has nowhere stable to attach:
...
CubeClawIO needs a single place to show:
...
- CubeClawIO-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without CubeClawIO owning the execution host
...
## 1. CubeClawIO host and execution host must be decoupled
...
without the CubeClawIO host itself running local git or local preview processes.
...
CubeClawIO should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for CubeClawIO.
...
CubeClawIO should keep workspace automation centered on a small set of hooks:
...
CubeClawIO should provide the equivalent visibility in the project `Code` surface:
...
CubeClawIO should treat execution workspaces as the natural home for:
...
CubeClawIO should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
CubeClawIO should continue treating cleanup policy as part of the core workspace model:
...
CubeClawIO should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the CubeClawIO host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

CubeClawIO should borrow Worktrunk's good execution patterns while keeping the broader CubeClawIO model:
...
This is how CubeClawIO supports:
...
That keeps CubeClawIO focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted CubeClawIO deployments remain first-class

That is a better fit for CubeClawIO than either extreme:
