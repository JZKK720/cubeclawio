...
`PAP-447` asks how CubeClawIO should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same CubeClawIO instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into CubeClawIO:

- some operators use CubeClawIO to manage CubeClawIO and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as CubeClawIO
- Claude and Codex expose different built-in affordances, so CubeClawIO should not overfit to one tool

## Core Product Decision

CubeClawIO should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by CubeClawIO
...
- CubeClawIO should not assume that every adapter can see or use a host filesystem path directly
...
If CubeClawIO makes worktrees agent-first, it will blur:
...
- CubeClawIO core resolves and records execution workspace state
...
That gives CubeClawIO room to support:
...
- CubeClawIO handles workspace runtime services under the hood using project/workspace policy
...
Once CubeClawIO is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So CubeClawIO should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that CubeClawIO cannot infer
...
- when issue moves to done, should CubeClawIO auto-commit?
- should it auto-push?
...
CubeClawIO already has the right foundation for a project-first model.
...
For server-spawned local adapters, CubeClawIO already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that CubeClawIO should mirror directly

Implication:

- for `codex_local`, CubeClawIO should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but CubeClawIO should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, CubeClawIO should separate:
...
CubeClawIO should be able to express intentions such as:
...
These are not specific to CubeClawIO. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a CubeClawIO-specific `devServer` flag.
...
CubeClawIO should distinguish between:
...
CubeClawIO should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, CubeClawIO should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to CubeClawIO
...
For runtime services specifically, CubeClawIO should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as CubeClawIO-only dev-server flags
...
- same embedded DB / CubeClawIO instance can be reused through env/config injection when appropriate
...
- CubeClawIO can decide whether to reuse or start a fresh service deterministically
...
3. Test against CubeClawIO-on-CubeClawIO workflows first.
...
7. Workspace runtime services are modeled generically, not as CubeClawIO-specific dev-server toggles.
