...
Date: 2026-03-13  
Related discussion: https://github.com/JZKK720/cubeclawio/discussions/449
...
- local runtime data from the default CubeClawIO instance on 2026-03-13
...
- CubeClawIO discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing CubeClawIO skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after CubeClawIO-side skill changes land.
...
- Store both:
  ...
  - CubeClawIO-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching CubeClawIO to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
- For `codex_local`, either:
  ...
  - treat CubeClawIO-owned Codex skill symlinks as repairable when they point at a different checkout
