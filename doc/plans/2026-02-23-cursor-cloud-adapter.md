...
This document defines the V1 design for a CubeClawIO adapter that integrates with
...
4. synthesized stdout events for CubeClawIO UI/CLI
...
3. **Agent callback auth to CubeClawIO** uses a bootstrap exchange flow (no long-lived CubeClawIO key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from CubeClawIO endpoints, not full SKILL.md prompt injection.
...
## CubeClawIO Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived CubeClawIO credentials in prompt text.

Flow:

1. Before launch/follow-up, CubeClawIO mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. CubeClawIO validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all CubeClawIO API calls.
...
1. Prompt includes a compact instruction to fetch skills from CubeClawIO.
...
4. Route by Cursor `agentId` to active CubeClawIO run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from CubeClawIO skill endpoints |
| CubeClawIO auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from CubeClawIO context.
3. Add richer PR metadata surfacing in CubeClawIO UI.
