...
CubeClawIO already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model CubeClawIO should implement next.

## Product Goals

CubeClawIO should let operators:
...
For the next implementation pass, CubeClawIO should enforce these defaults:
...
CubeClawIO should separate subscription-included usage from billed spend:
...
CubeClawIO should have two threshold classes:
...
CubeClawIO must know that a project is budget-paused and enforce it.
...
CubeClawIO must also block execution before new work starts.
