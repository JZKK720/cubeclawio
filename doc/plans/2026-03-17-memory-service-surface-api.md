# CubeClawIO Memory Service Plan

## Goal

Define a CubeClawIO memory service and surface API that can sit above multiple memory backends, while preserving CubeClawIO's control-plane requirements:

- company scoping
- auditability
- provenance back to CubeClawIO work objects
- budget / cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md` and on the current CubeClawIO architecture in:
...
CubeClawIO should not embed one opinionated memory engine into core. It should add a company-scoped memory control plane with a small normalized adapter contract, then let built-ins and plugins implement the provider-specific behavior.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing CubeClawIO plugin runtime.

That keeps the core small and matches the current direction that optional knowledge-like systems live at the edges.

### 4. CubeClawIO owns routing, provenance, and accounting

Providers should not decide how CubeClawIO entities map to governance.

CubeClawIO core should own:
...
The normalized CubeClawIO scope passed into a provider request.
...
A normalized write, query, browse, or delete action performed through CubeClawIO.

CubeClawIO should log every operation, whether the provider is local or external.
...
## What CubeClawIO Should Persist

CubeClawIO should not mirror the full provider memory corpus into Postgres unless the provider is a CubeClawIO-managed local provider.

CubeClawIO core should persist:
...
1. `pre-run hydrate`
   Before an agent run starts, CubeClawIO may call `query(... intent = "agent_preamble")` using the active binding.

2. `post-run capture`
   After a run finishes, CubeClawIO may write a summary or transcript-derived note tied to the run.

3. `issue comment / document capture`
   When enabled on the binding, CubeClawIO may capture selected issue comments or issue documents as memory sources.
...
CubeClawIO should give agents both automatic recall and explicit tools, with simple guidance:
...
CubeClawIO needs a first-class UI for memory, otherwise providers become black boxes.
...
This is important because a memory system that "works" but silently burns budget is not acceptable in CubeClawIO.
...
- it matches CubeClawIO's local-first posture
- it is inspectable
...
- Do we want CubeClawIO-managed extraction pipelines at all, or should built-ins be the only place where CubeClawIO owns extraction?
...
- CubeClawIO owns memory bindings, scopes, provenance, governance, and usage reporting.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives CubeClawIO a stable "memory service" without locking the product to one memory philosophy or one vendor.
