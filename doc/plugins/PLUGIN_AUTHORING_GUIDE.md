...
```bash
pnpm --filter @cubeclawhub/create-paperclip-plugin build
...
Inside this monorepo, the scaffold uses `workspace:*` for `@cubeclawhub/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@cubeclawhub/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@cubeclawhub/plugin-sdk/ui`
