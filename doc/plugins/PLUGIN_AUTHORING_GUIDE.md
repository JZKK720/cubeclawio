...
This guide describes the current, implemented way to create a CubeClawIO plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main CubeClawIO app.
...
Inside this monorepo, the scaffold uses `workspace:*` for `@cubeclawio/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@cubeclawio/plugin-sdk` from the local CubeClawIO checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into CubeClawIO from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@cubeclawio/plugin-sdk/ui`
