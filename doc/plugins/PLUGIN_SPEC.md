# CubeClawIO Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for CubeClawIO's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main CubeClawIO app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary CubeClawIO HTTP APIs directly.
...
CubeClawIO plugin design is based on the following assumptions:

1. CubeClawIO is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core CubeClawIO business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by CubeClawIO core.
...
2. Let plugins add major capabilities without editing CubeClawIO core.
...
The single CubeClawIO deployment an operator installs and controls.

### 5.2 Company

A first-class CubeClawIO business object inside the instance.
...
A trusted in-process extension loaded directly by CubeClawIO core.
...
An installable instance-wide extension package loaded through the CubeClawIO plugin runtime.
...
CubeClawIO has two extension classes.
...
CubeClawIO already has a concrete workspace model:
...
Plugins live under the CubeClawIO instance directory.
...
CubeClawIO should add CLI commands:

- `pnpm cubecloud.io plugin list`
- `pnpm cubecloud.io plugin install <package[@version]>`
- `pnpm cubecloud.io plugin uninstall <plugin-id>`
- `pnpm cubecloud.io plugin upgrade <plugin-id> [version]`
- `pnpm cubecloud.io plugin doctor <plugin-id>`
...
```ts
export interface CubeClawIOPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumCubeClawIOVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that CubeClawIO agents can use during runs.
...
- CubeClawIO server starts one worker process per installed plugin
...
Receives one typed CubeClawIO domain event.
...
export function definePlugin(definition: PluginDefinition): CubeClawIOPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: CubeClawIOPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@cubeclawio/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@cubeclawio/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@cubeclawio/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@cubeclawio/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@cubeclawio/plugin-sdk/ui`
...
The `@cubeclawio/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the CubeClawIO secret provider system rather than a plain text input
...
1. Core CubeClawIO data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing CubeClawIO objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual CubeClawIO product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if CubeClawIO later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing CubeClawIO secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm cubecloud.io plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the CubeClawIO server. This is a normative requirement, not optional.
...
### 27.1 `@cubeclawio/plugin-test-harness`
...
```ts
import { createTestHarness } from "@cubeclawio/plugin-test-harness";
...
For developing a plugin against a running CubeClawIO instance:

- The operator installs the plugin from a local path: `pnpm cubecloud.io plugin install ./path/to/plugin`
...
- `@cubeclawio/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@cubeclawio/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@cubeclawio/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@cubeclawio/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@cubeclawio/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@cubeclawio/plugin-sdk/ui`
...
- `@cubeclawio/plugin-test-harness`
...
CubeClawIO should not implement a generic in-process hook bag modeled directly after local coding tools.

CubeClawIO should implement:
...
That is the complete target design for the CubeClawIO plugin system.
