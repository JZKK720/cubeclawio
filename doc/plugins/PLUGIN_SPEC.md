...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@cubeclawhub/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@cubeclawhub/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@cubeclawhub/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@cubeclawhub/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@cubeclawhub/plugin-sdk/ui`
...
The `@cubeclawhub/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
### 27.1 `@cubeclawhub/plugin-test-harness`
...
```ts
import { createTestHarness } from "@cubeclawhub/plugin-test-harness";
...
- `@cubeclawhub/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@cubeclawhub/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@cubeclawhub/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@cubeclawhub/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@cubeclawhub/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@cubeclawhub/plugin-sdk/ui`
...
- `@cubeclawhub/plugin-test-harness`
