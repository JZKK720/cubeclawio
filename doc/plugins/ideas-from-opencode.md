...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@cubeclawhub/plugin-sdk/ui`) with design tokens and common components.
...
A `@cubeclawhub/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Paperclip instance.
...
- **Single SDK package**: `@cubeclawhub/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@cubeclawhub/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
```ts
import { definePlugin, z } from "@cubeclawhub/plugin-sdk";
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@cubeclawhub/plugin-sdk/ui";
...
- plugin UI bundle loading, host bridge, `@cubeclawhub/plugin-sdk/ui`
...
- `@cubeclawhub/plugin-test-harness` and `create-paperclip-plugin` starter template
...
8. Ship `@cubeclawhub/plugin-test-harness` and `create-paperclip-plugin` starter template.
