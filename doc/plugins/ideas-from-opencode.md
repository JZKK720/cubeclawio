...
CubeClawIO V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a CubeClawIO-shaped design.

Assumption for this document: CubeClawIO is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class CubeClawIO objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into CubeClawIO.

The main conclusion is:

- CubeClawIO should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- CubeClawIO should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- CubeClawIO should use multiple extension classes instead of one generic plugin bag:
...
If CubeClawIO does this well, the examples you listed become straightforward:
...
Relevant CubeClawIO files reviewed for current extension seams:
...
It is too dangerous for CubeClawIO core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for CubeClawIO — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
CubeClawIO should absolutely do this.
...
CubeClawIO should copy this discipline.
...
CubeClawIO should adopt this style for plugin actions, automations, and UI schemas.
...
CubeClawIO can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for CubeClawIO too.

## What CubeClawIO Should Not Copy Directly
...
CubeClawIO is a control plane for an operator-managed instance with company objects.
...
CubeClawIO is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

CubeClawIO should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For CubeClawIO, equivalent hooks into:
...
CubeClawIO should not allow plugins to silently replace:
...
For CubeClawIO it would be risky because it combines:
...
CubeClawIO should require an explicit operator install step.

## Why CubeClawIO Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | CubeClawIO |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted CubeClawIO instance |
...
That means CubeClawIO should borrow the good ideas from `opencode` but use a stricter architecture.

## CubeClawIO Already Has Useful Pre-Plugin Seams

CubeClawIO has several extension-like seams already:
...
This is good news.
CubeClawIO does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended CubeClawIO Plugin Model
...
CubeClawIO already has this pattern implicitly:
...
- `plugin` means integration code that talks to CubeClawIO through a typed plugin protocol and capability model
...
- invoke explicit CubeClawIO actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@cubeclawio/plugin-sdk/ui`) with design tokens and common components.
...
CubeClawIO should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over CubeClawIO objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

CubeClawIO already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for CubeClawIO too.
...
A `@cubeclawio/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running CubeClawIO instance.

A `create-paperclip-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how CubeClawIO achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the CubeClawIO server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. CubeClawIO's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@cubeclawio/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@cubeclawio/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For CubeClawIO

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@cubeclawio/plugin-sdk";
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit CubeClawIO entities
    });
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@cubeclawio/plugin-sdk/ui";
...
Any CubeClawIO plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific CubeClawIO objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of CubeClawIO itself
...
If a concept is becoming part of CubeClawIO's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of CubeClawIO itself
...
### 3. Scope plugin data to CubeClawIO objects before adding custom schemas

A lot of plugin data naturally hangs off existing CubeClawIO objects:
...
If CubeClawIO eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving CubeClawIO.
- Board reviews the diff after a run without leaving CubeClawIO.
...
This plugin syncs CubeClawIO work with Linear. It is useful for:

- importing backlog from Linear
- linking CubeClawIO issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between CubeClawIO and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into CubeClawIO.
- CubeClawIO issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs CubeClawIO issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to CubeClawIO issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with CubeClawIO task governance
...
- Plugin settings:
  ...
  - whether new CubeClawIO issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active CubeClawIO issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into CubeClawIO.
- GitHub webhooks update status/comment state in CubeClawIO.
- A PR is linked back to the CubeClawIO issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside CubeClawIO. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating CubeClawIO issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full CubeClawIO plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the CubeClawIO dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a CubeClawIO issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of CubeClawIO core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into CubeClawIO. It is useful for:
...
- Failed payment spikes or churn events can generate CubeClawIO issues for follow-up.
...
- Stripe data should stay additive to CubeClawIO core
...
- plugin UI bundle loading, host bridge, `@cubeclawio/plugin-sdk/ui`
...
- `@cubeclawio/plugin-test-harness` and `create-paperclip-plugin` starter template
...
CubeClawIO should not implement "an OpenCode-style generic in-process hook system."
CubeClawIO should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In CubeClawIO
...
8. Ship `@cubeclawio/plugin-test-harness` and `create-paperclip-plugin` starter template.
