...
Audience: Operators setting up and running agents in CubeClawIO

## 1. What this system does

Agents in CubeClawIO do not run continuously.  
...
For `claude_local` and `codex_local`, CubeClawIO assumes the CLI is already installed and authenticated on the host machine.
...
CubeClawIO stores resumable session state per `(agent, taskKey, adapterType)`.
...
CubeClawIO pushes runtime/activity updates to the browser in real time.
