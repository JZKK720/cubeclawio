...
This spec defines how CubeClawIO actually runs agents while staying runtime-agnostic.
...
1. CubeClawIO is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, CubeClawIO manages lifecycle, and on exit CubeClawIO parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared CubeClawIO skill text block)
