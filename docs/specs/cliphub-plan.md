# ClipHub: Marketplace for CubeClawIO Team Configurations

> The "app store" for whole-company AI teams — pre-built CubeClawIO configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for CubeClawIO-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | CubeClawIO company (agents, projects, governance) |
...
A complete CubeClawIO company configuration:
...
Single-agent configurations designed to plug into a CubeClawIO org:
...
Portable skill files that any CubeClawIO agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with CubeClawIO's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum CubeClawIO version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target CubeClawIO company
...
| `POST` | `/api/listings/:id/install` | Install to CubeClawIO company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target CubeClawIO company (or create new)
  → ClipHub API calls CubeClawIO API to:
  ...
  → Redirect to CubeClawIO dashboard with new team running
```
...
### 5.3 Creator: Export from CubeClawIO → Publish

```
Running CubeClawIO company → "Export as Blueprint" (CLI or UI)
  → CubeClawIO exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (CubeClawIO brand blue/purple)
...
| Install flow | One-click deploy to CubeClawIO company |
...
{
  "targetCompanyId": "uuid",         // Existing CubeClawIO company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as CubeClawIO
- **Backend**: Node.js API (or extend CubeClawIO server)
- **Database**: Postgres (can share CubeClawIO's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with CubeClawIO auth (or OAuth2)

### 9.2 Integration with CubeClawIO

ClipHub can be:
- **Option A**: A separate app that calls CubeClawIO's API to install blueprints
- **Option B**: A built-in section of the CubeClawIO UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing CubeClawIO UI and API.
...
- [ ] Install flow: blueprint → CubeClawIO company
...
- [ ] "Export from CubeClawIO" CLI command
