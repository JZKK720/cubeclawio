...
Build a custom adapter to connect CubeClawIO to any agent runtime.
...
2. Build environment with `buildCubeClawIOEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `cubecloud.io run --watch` using `picocolors`.
...
Make CubeClawIO skills discoverable to your agent runtime without writing to the agent's working directory:
