...
Add first-class **documents** to CubeClawIO as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by CubeClawIO workflow/docs.
...
Update the CubeClawIO agent workflow so planning no longer edits the issue description.
