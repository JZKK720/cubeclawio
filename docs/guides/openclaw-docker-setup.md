...
How to get OpenClaw running in a Docker container for local development and testing the CubeClawIO OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

CubeClawIO includes an end-to-end join smoke harness:
...
- probes and prints a CubeClawIO host URL that is reachable from inside OpenClaw Docker
...
If your CubeClawIO deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host CubeClawIO server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed CubeClawIO URL (typically `http://host.docker.internal:3100`).
- If CubeClawIO rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm cubecloud.io allowed-hostname host.docker.internal
```

Then restart CubeClawIO and rerun the smoke script.
...
```bash
pnpm cubecloud.io allowed-hostname <host>
```
