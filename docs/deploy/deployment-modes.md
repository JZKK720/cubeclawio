...
CubeClawIO supports two runtime modes with different security profiles.
...
```sh
pnpm cubecloud.io allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, CubeClawIO emits a one-time claim URL at startup:
...
```sh
PAPERCLIP_DEPLOYMENT_MODE=authenticated pnpm cubecloud.io run
```
