...
## 2. Shared Types (`@cubeclawhub/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@cubeclawhub/adapter-utils` (types) or `@cubeclawhub/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  ...
  "dependencies": {
    "@cubeclawhub/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@cubeclawhub/adapter-utils/server-utils`)
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@cubeclawhub/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@cubeclawhub/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@cubeclawhub/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@cubeclawhub/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@cubeclawhub/adapter-my-agent/cli";
...
Import from `@cubeclawhub/adapter-utils/server-utils`:
...
- Package name: `@cubeclawhub/adapter-<kebab-name>`
