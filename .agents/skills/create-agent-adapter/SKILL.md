...
## 2. Shared Types (`@cubeclawio/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@cubeclawio/adapter-utils` (types) or `@cubeclawio/adapter-utils/server-utils` (runtime helpers).
...
```json
{
  ...
  "dependencies": {
    "@cubeclawio/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@cubeclawio/adapter-utils/server-utils`)
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@cubeclawio/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@cubeclawio/adapter-my-agent";
...
import { parseMyAgentStdoutLine } from "@cubeclawio/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@cubeclawio/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@cubeclawio/adapter-my-agent/cli";
...
Import from `@cubeclawio/adapter-utils/server-utils`:
...
- Package name: `@cubeclawio/adapter-<kebab-name>`
