 ...
 *
 * Bundles all workspace packages (@cubeclawhub/*) into a single file.
...
// to npm and resolved at runtime (e.g. @cubeclawhub/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@cubeclawhub/")) {
      externals.add(name);
    }
  }
