...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" cubecloud.io onboard ")));
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@cubeclawio/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `cubecloud.io doctor`"));
      }
    }
  ...
  p.note(
    [
      `Run: ${pc.cyan("cubecloud.io run")}`,
      `Reconfigure later: ${pc.cyan("cubecloud.io configure")}`,
      `Diagnose setup: ${pc.cyan("cubecloud.io doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("cubecloud.io run")}`,
        `Then: ${pc.cyan("cubecloud.io auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
