  ...
  const packageJson = {
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@cubeclawhub/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@cubeclawhub/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { PaperclipPluginManifestV1 } from "@cubeclawhub/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@cubeclawhub/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@cubeclawhub/plugin-sdk\` and \`@cubeclawhub/shared\` from a local Paperclip checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.paperclip-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@cubeclawhub/plugin-sdk/bundlers\`.
